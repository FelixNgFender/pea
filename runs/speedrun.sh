...
# Default intermediate artifacts directory is in ./peashooter
export OMP_NUM_THREADS=1
export PEASHOOTER_BASE_DIR="./peashooter"
mkdir -p "$PEASHOOTER_BASE_DIR"
...
uv run scripts/fineweb_chat.py -n 8
...
uv run scripts/fineweb_chat.py -n 370 &
DATASET_DOWNLOAD_PID=$!
# train the tokenizer with vocab size 2**15 = 32768 on ~2B characters of data
pealm train peashooter-tokenizer
# evaluate the tokenizer (report compression ratio etc.)
pealm eval peashooter-tokenizer
...
# curl -L -o "$PEASHOOTER_BASE_DIR"/identity_conversations.jsonl https://karpathy-public.s3.us-west-2.amazonaws.com/identity_conversations.jsonl
#
